...
#Put the plots in an 'if statement' depending on the number of 
#predictor variables detected
...
if(length(predictors)==2){
  cat("More than two predictor variables detected. Generating plots for 
      first two predictors only.\n")
  #predictors <- predictors[1:2]
  if (length(time_var) > 0) {
    for (v in time_var) {
      p1 <- ggplot(my_data, aes_string(x = v, y = response_var, 
                                       color = treatment_var)) +
  ...
  cat("Three or more predictor variables detected. Generating plots for 
      first three predictors only.\n")
  for (v in time_var) {
    p2 <- ggplot(my_data, aes_string(x = v, y = response_var, 
                                     color = treatment_var)) +
      geom_point(aes(group = treatment_var), alpha = 0.6, size = 2) +
      theme_minimal(base_size = 14) +
...
}else{ stop("One or two predictor variables detected. Generating plots 
            accordingly.\n")}

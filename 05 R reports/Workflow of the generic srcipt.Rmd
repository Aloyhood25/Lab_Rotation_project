...
Now create a working directory for our project with folders for files 
```{r}
...
The code above creates a folder on your desktop. YOu go ahead and add the csv file for your data in the raw files folder 
...
We need to identify the response variable from our data using keyword 'sperm motility' and set it apart from other variables 
```{r}
...
Now our variables have been identified and classified. We can go ahead and produce some visualisations for our data. 
```{r}
...
Having made exploratory plots. We might want to a little further and plot the group means, max and min points and standard error.
...
Using DARHMa library from the emmeans package, we can run some model diagnostics to make sure our model properly reperesents our data 
```{r}
